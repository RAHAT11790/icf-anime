...
const LiveSupportChat = ({ getAnimeList, isOpen, onClose, onAnimeSelect }: LiveSupportChatProps) => {
  ...
  const buildAnimeContext = useCallback(() => {
    ...
    context += `ICF Catalog (${primaryItems.length}টি):\n`;
  ...
  const sendMessage = async () => {
    ...
    if (text.includes("@ICF") || text.includes("@icf") || text.includes("@ICF")) {
    ...
    try {
      ...
      const errMsg: ChatMessage = {
        ...
        content: isRateLimit
          ...
          : "⚠️ সার্ভারে সমস্যা হচ্ছে। একটু পরে আবার চেষ্টা করুন। সরাসরি Admin-এর কাছে পৌঁছাতে @ICF লিখে মেসেজ করুন।",
        timestamp: Date.now(),
      };
  ...
  return (
    <div className="fixed left-2 right-2 sm:left-auto sm:right-3 sm:w-[370px] z-[60] rounded-2xl overflow-hidden flex flex-col bg-background"
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-[10px] text-primary/60 mt-2">Admin-এর সাথে কথা বলতে @ICF লিখুন</p>
          </div>
        )}
        {messages.map((msg) => (
      ...
      <div className="p-3" style={{ boxShadow: "0 -2px 8px rgba(0,0,0,0.04)" }}>
        ...
        <p className="text-[9px] text-muted-foreground text-center mt-2">@ICF লিখে Admin-কে সরাসরি মেসেজ করুন</p>
      </div>
    </div>
  );
};
