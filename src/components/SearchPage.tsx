                ...
                {historyAnime.map((anime) => (
                  <div key={anime.id} className="relative aspect-[2/3] rounded-xl overflow-hidden cursor-pointer poster-hover bg-card" style={{ boxShadow: "var(--neu-shadow-sm)" }} onClick={() => handleCardClick(anime)}>
                    ...
                    <div className="absolute top-1.5 left-1.5 flex flex-col gap-1 z-10">
                      ...
                      <span className={`px-1.5 py-0.5 rounded text-[7px] font-black tracking-wider ${anime.source === "animesalt" ? "bg-accent/85 text-accent-foreground" : "bg-primary/85 text-primary-foreground"}`}>{anime.source === "animesalt" ? "AN" : "ICF"}</span>
                    </div>
